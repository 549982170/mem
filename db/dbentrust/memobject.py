...
class MemObject:
    ...
    def produceKey(self, keyname):
        ...
        if isinstance(keyname, basestring):
            return str(''.join([self._name, ':', keyname]))  # 生成如"character2:id" 的keyname
        else:
            raise "type error"
    ...
    def insert(self):
        ...
        newmapping = dict(zip([self.produceKey(keyname) for keyname in nowdict.keys()],
                              nowdict.values()))
        # newmapping like
        # dict: {'character2:id': 2L, 'character2:_lock': 0, 'character2:_name': 'character2', 'character2:nickname': u'fage407', 'character2:profession': 0, 'character2:guanka': None, 'character2:level': 1L}
        #         logger.debug('insert data: %s' % newmapping)
